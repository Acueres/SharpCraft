...
record struct WorkItem(Vec3<int> Index, int Version);
...
class ChunkDispatcher : IDisposable, IAsyncDisposable
{
    ...
    private void HandleSuccess(ChunkRecord record, WorkResult result)
    {
        ...
        switch (result.JobType)
        {
            case JobType.Generation:
                ...
                region[result.Index] = result.Chunk;
    ...
    private void ProcessDeletion(List<Vec3<int>> idxDeletion)
    {
        foreach (var id in idxDeletion)
        {
            ...
            if (record.Chunk is { } chunk)
            {
                ...
                if (!region.ContainsColumn(chunk.Index.X, chunk.Index.Z))
    ...
    private bool TryGetChunk(Vec3<int> index, int version, out Chunk chunk)
    {
        chunk = null;
